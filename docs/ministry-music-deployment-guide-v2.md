# Deploying Ministry Collection + Ambient Reading Player

## Overview

Two features in a single deployment:

**Feature 1 — Music Page: Ministry Collection**
...
## Step 2: Music Page — Add Ministry Collection

### Claude Code Prompt

```
Update src/pages/music.njk to add a "Ministry Collection" section before the
...
2. Playlist order becomes:
   Section 0: Ministry Collection (from ministryMusic.collection, in array order)
   Section 1: The Core Journey (chapter primaries, unchanged)
   Section 2: More Arrangements (chapter alternates, unchanged)

TRACK LIST DISPLAY:

3. Add a new section header BEFORE "The Core Journey" header:
   - Header text: "Ministry Collection"
...
7. Ministry Collection tracks participate fully in shuffle. When shuffle is
...
- [ ] "Ministry Collection" section appears first in track list
...
```
Stage and commit all changes for the Ministry Collection + Ambient Reader:
...
Modified files:
- src/pages/music.njk (Ministry Collection section)
- src/js/music-player.js (if changes needed for isMinistry flag)
- src/css/music.css (if new styles for Ministry Collection)
- src/_includes/layouts/chapter.njk (ambient control HTML + data injection)
- src/_includes/layouts/base.njk (ambient CSS/JS links)

Commit message:
"feat: Ministry Collection on Music page + ambient reading player on chapters

Music page:
- New ministryMusic.json data file for non-chapter audio
- Ministry Collection (anthem + symphony) plays before chapter journey
