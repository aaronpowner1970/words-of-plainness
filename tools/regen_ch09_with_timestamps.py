"""
Ch. 9 chunk regeneration with word-level timestamps.

Calls ElevenLabs /v1/text-to-speech/{voice_id}/with-timestamps for each
chunk, capturing both the audio bytes and character/word alignment data
in a single API call. This is the correct professional approach — timestamps
are produced by the same synthesis engine that renders the audio, giving
millisecond accuracy.

Outputs:
  - Chunk MP3 files -> CHUNKS_DIR (overwrites previous)
  - Per-chunk alignment JSON -> CHUNKS_DIR/alignment/chunk_NN.json
  - Final timestamp JSON -> OUTPUT_PATH (ready for deployment)

Usage:
    python tools\regen_ch09_with_timestamps.py

You will be prompted for your ElevenLabs API key once.
"""
...
VOICE_JONATHAN = "PIGsltMj3gFMR34aFDI3"
MODEL_ID     = "eleven_multilingual_v2"

# Voice settings — Aaron prose
AARON_SETTINGS = {
    "stability": 0.62,
...
His character is not the absence of human feeling but the perfection of it: every impulse governed by love, every response shaped by wisdom, every choice submitted to the Father. He is the mirror we hold up to see ourselves honestly."""),
    ...
    "03": (VOICE_AARON, AARON_SETTINGS, """Yehoshua spent roughly thirty years doing manual labor and only three years in ministry. That ratio itself is a testament to His humility. God in the flesh chose to quietly shape wood and stone in the privacy of an insignificant village before He ever shaped souls and performed miracles publicly.
...
My mother used to remind me that one does not announce one's own humility. Throughout my life, I have never felt safer than when I am with genuinely humble people. Sensing no competition, no threat, it allows me to let down my guard. Humility in others allows us to see deeper than the facade of ego to real depth of character. This is what it must have felt like to be near Yehoshua. To some it may have been frightening or suspicious; to disciples it feels like coming home."""),
    ...
    "04": (VOICE_JONATHAN, JONATHAN_SETTINGS, "This is a good moment to pause. The reflection tabs in the margin invite you to sit with what you just read before we continue. Free registration saves all your reflection work to a personal report you can return to anytime from the user menu."),

    "05": (VOICE_AARON, AARON_SETTINGS, """Many Christians identify Christ's ultimate act of obedience to the Father by the words of surrender in Gethsemane: "Not my will, but thine, be done." This moment will be studied in another chapter. Here, let us observe His daily examples of obedience as evidence of His character.
...
Throughout His mortal ministry, the Lord was filled with compassion for souls who "fainted, and were scattered abroad, as sheep having no shepherd." """),
    ...
    "09": (VOICE_AARON, AARON_SETTINGS, """This trait did not lessen or end with His death. In the Eastern Hemisphere, the resurrected Master comforted the weeping Mary at His empty tomb. He reassured the fearful disciples. He removed Peter's shame by allowing him to declare his love three times, once for each time he had previously denied Him. He walked with the grieving disciples on the road to Emmaus. He personally made a warm fire at a campsite and hand cooked a meal for His disciples who were returning from a frustrating day of fishing.
...
I remember a moment when a teenage special education student said something so disrespectful in my science laboratory that every instinct told me to respond by matching his energy. Instead I bit my tongue, threw a desperate prayer heavenward, and waited. The Spirit softened my heart and opened my eyes — not just to find compassion for the student, but to see how to reach him. The unmet need motivating his behavior became the focus for the teaching moment. I learned that day what the Master practiced perfectly: truth that reaches a soul must first be calibrated to the soul it is reaching. This is not just a teaching technique; it flows from habits of grace that we learn from Yehoshua the man."""),
    ...
    "12": (VOICE_AARON, AARON_SETTINGS, """The same hands that overturned tables in the temple at Jerusalem also washed feet in the prepared upper room. Why? It bears repeating: Yehoshua was full of grace and truth.
...
I pray that I am developing a measure of Christlike character as His grace and truth work upon my spirit. I imitate Him imperfectly, but it is my covenant duty to try. There is comfort for me in holding up the character of Christ as a mirror to examine my soul. I remind myself often not to focus on how far I fall short of Him, but to hold to my faith that His grace sees me as I hope to be, and that His spirit works tirelessly upon me daily to close the gap."""),
    ...
    "13": (VOICE_AARON, AARON_SETTINGS, """The Pharisees and their entourage of Scribes dragged her into the temple courts on a morning when Yehoshua sat teaching a gathering of the people. They interrupted His teaching and thrust her in the middle of His gathering.
...
In all our spiritual journeys, when faced with the opportunities to judge one another, let us walk in the grace of His light. Anything else is walking in the dark."""),
    ...
    "14": (VOICE_AARON, AARON_SETTINGS, """Contrasting His own lifestyle to that of His cousin who taught in the wilderness, Yehoshua described Himself as eating and drinking with His followers.
...
I remember a full day of shoveling manure with fellow saints for neighborhood gardens. We didn't dare hug or even shake hands, smiling brightly despite being thoroughly covered in muck. How odd it is that we can be covered in filth while filled with joy in the service of the Master and while caring for one another. This truly is the abundant life He promised."""),
    ...
    "15": (VOICE_AARON, AARON_SETTINGS, """This chapter could not possibly detail all the marvelous personality traits that flow from Yehoshua's core of grace and truth. The Apostle John also felt the need to explain: "There are also many other things which Jesus did, the which, if they should be written every one, I suppose that even the world itself could not contain the books that should be written."
...
That same grace that is the living core of His character is a power at work in every willing disciple. Studying His character is not meant to measure the distance between Him and us — it is meant to show us what we can become as His grace works its refining purpose in our mortal lives. Beholding the truth of His grace as the mechanism of our own transformation is the whole point of this chapter."""),
    ...
    "16": (VOICE_AARON, AARON_SETTINGS, """You've sat with Yehoshua. You have studied the Man who is God. You've observed that His great heart — full of grace — beats with His life-giving truth throughout all parts of His character. We hear the phrase "He died for us" often, but let us not forget that He also lives for us. "He is the Lord of both the dead and the living." His heart still beats for you.
...
In the name of Jesus Christ, Amen."""),
    ...
    "17": (VOICE_JONATHAN, JONATHAN_SETTINGS, "Before we close — the reflection tabs in the margin are waiting. What you just read deserves more than a moment. Free registration saves all your reflection work to a personal report in your user menu."),
}

# ── Chunk to sentence range mapping ──────────────────────────────────────
# (first_sentence, last_sentence, is_cue, cue_index)
CHUNK_SENTENCES = {
    "01": (0,    27,   False, None),
    "02": (28,   61,   False, None),
    "03": (62,   85,   False, None),
...
def call_tts_with_timestamps(api_key, voice_id, settings, text):
    ...
    Returns (audio_bytes, alignment_data) or raises on error.
    alignment_data is the full JSON response dict.
    """
    ...
    #   alignment: {
    #     characters: [str, ...]
    #     character_start_times_seconds: [float, ...]
...
def alignment_to_word_times(alignment):
    """
    Convert character-level alignment to word-level start times.
    ...
    # Join characters into words, tracking start/end of each word
    words = []
    current_word = ""
    word_start = None
    word_end = None
    ...
    for i, ch in enumerate(chars):
        ...
        if ch in (" ", "\n", "\t"):
            if current_word.strip():
                words.append({"word": current_word.strip(), "start": word_start, "end": word_end})
            current_word = ""
            word_start = None
        else:
...
def find_sentence_start(sent_text, word_times, search_from=0):
    """
    Find the start time of a sentence within word_times.
    Returns (start_time_seconds, next_word_ptr).
    """
...
print(f"\n=== Step 3: Output directories ready ===")
print(f"  Chunks : {CHUNKS_DIR}")
print(f"  Alignment: {ALIGNMENT_DIR}")

# ── Step 4: Generate chunks and collect timestamps ─────────────────────────

print(f"\n=== Step 4: Generating {len(CHUNK_TEXTS)} chunks with timestamps ===")

timestamps = {}
cumulative_offset = 0.0
chunk_order = sorted(CHUNK_TEXTS.keys())

for chunk_id in chunk_order:
    voice_id, settings, text = CHUNK_TEXTS[chunk_id]
    first_sent, last_sent, is_cue, cue_idx = CHUNK_SENTENCES[chunk_id]

    chunk_filename = f"ch09_chunk_{chunk_id}.mp3"
    chunk_path = os.path.join(CHUNKS_DIR, chunk_filename)
    alignment_path = os.path.join(ALIGNMENT_DIR, f"chunk_{chunk_id}.json")
    voice_label = "Aaron" if voice_id == VOICE_AARON else "Jonathan"

    print(f"\n  Chunk {chunk_id} [{voice_label}] | offset: {cumulative_offset:.3f}s")

    try:
        audio_bytes, alignment = call_tts_with_timestamps(api_key, voice_id, settings, text)
    ...
    # Get actual duration from the saved file
    chunk_dur = get_duration(chunk_path)
    print(f"    Saved: {chunk_filename} ({chunk_dur:.2f}s, {len(audio_bytes):,} bytes)")

    if is_cue:
        # Cue: timestamp at offset + 0.5s
        cue_time = round(cumulative_offset + 0.5, 3)
        timestamps[str(cue_idx)] = cue_time
        print(f"    CUE {cue_idx} -> {cue_time}s")
    else:
        # Prose: align sentences to word timestamps
        word_times = alignment_to_word_times(alignment)
        print(f"    Word-level timestamps: {len(word_times)} words")
        ...
        print(f"    s{first_sent}: {timestamps[str(first_sent)]}s  |  s{last_sent}: {timestamps[str(last_sent)]}s")

    cumulative_offset = round(cumulative_offset + chunk_dur, 3)

# ── Step 5: Build concat list and assemble final MP3 ──────────────────────

print(f"\n=== Step 5: Building assembly concat list ===")
concat_path = os.path.join(CHUNKS_DIR, "concat_new.txt")
with open(concat_path, "w") as f:
    for chunk_id in chunk_order:
        f.write(f"file 'ch09_chunk_{chunk_id}.mp3'\n")
print(f"  Written: {concat_path}")
print(f"\n  Run this command to assemble the final MP3:")
print(f"  cd \"{CHUNKS_DIR}\"")
print(f"  ffmpeg -f concat -safe 0 -i concat_new.txt -af loudnorm=I=-16:TP=-1.5:LRA=11 -codec:a libmp3lame -qscale:a 2 NR_09_01_Yehoshua_the_Man.mp3")

# ── Step 6: Sort and save timestamp JSON ──────────────────────────────────

print(f"\n=== Step 6: Saving timestamp JSON ===")
ts_sorted = {str(k): timestamps[str(k)]
             for k in sorted(int(k) for k in timestamps.keys())}
...
for i in checks:
    ...
    if key in ts_sorted:
        ...
        print(f"  [{key:>3}] {t:>8.3f}s  ({mins}:{secs:06.3f})  {preview}")

print(f"\nDone. Assembly command above, then upload NR_09_01_Yehoshua_the_Man.mp3 to R2.")
