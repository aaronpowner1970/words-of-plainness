"""
build_section_timestamps.py  (WoP Ch. 9 — per-section timestamp builder)

PURPOSE
=======
Build four per-section timestamp JSON files for the Ch. 9 section-based
audio architecture. Each section has its own assembled MP3 (S01–S04) and
its own output JSON (chapter-09-s01.json through chapter-09-s04.json).

The approach is identical to convert-to-sentence-timestamps.py:
  - Character-walk alignment (char_pos_to_time) — NOT ratio alignment
  - Proportional scaling: chunk share of section = chunk_pre_dur / sum_pre_durs_in_section
  - First prose paragraph of each section forced to 0.0 (silence pre-roll)

SECTION → CHUNK MAPPING:
  S01  chunks 01, 02, 03  sentences 0–85    paragraphs 1–23
  S02  chunks 05, 06      sentences 86–150  paragraphs 25–40
  S03  chunks 08, 09      sentences 151–199 paragraphs 42–55
  S04  chunks 11–16       sentences 200–384 paragraphs 57–115

NOTE: Cue chunks (04, 07, 10, 17) are skipped entirely.
No "cue{N}" keys in per-section JSONs — cue state is handled by the
state machine's 'ended' event, not timestamp polling within section files.

OUTPUT FORMAT (scoped to each section):
  "p{N}"  -> paragraph start time within the section file (seconds)
  "s{N}"  -> paragraph index for sentence N (click-to-seek lookup)

BUG FIX (March 2026):
  find_in_narration() now returns a `found` boolean as its third value.
  On a FAILED match the pointer is NOT advanced — it stays at search_from.
  This prevents heading sentences (never narrated, absent from chunk text)
  from false-matching a shared word that appears later in the text, which
  would push search_from past the correct position of all following sentences.
  Example: s62 "Mastery of Humility: Grace Embodied" — "mastery" falsely
  matched "...proclaim His mastery of humility..." at char 1757, displacing
  s63-s79 to the wrong time region. Fixed by not advancing on failure.

USAGE
=====
  python tools/build_section_timestamps.py

Idempotent — safe to re-run. Writes directly to:
  src/_data/timestamps/chapter-09-s01.json
  src/_data/timestamps/chapter-09-s02.json
  src/_data/timestamps/chapter-09-s03.json
  src/_data/timestamps/chapter-09-s04.json
"""
...
TIMESTAMPS_DIR = r"C:\Users\aaron\Documents\words-of-plainness\src\_data\timestamps"

# Section definitions:
# (section_id, label, prose_chunk_ids, sent_start, sent_end, para_start, para_end)
SECTIONS = [
    ("s01", "Invocation + Humility",  ["01","02","03"],             0,   85,  1,  23),
    ("s02", "Obedience",              ["05","06"],                  86,  150, 25, 40),
    ("s03", "Compassion",             ["08","09"],                  151, 199, 42, 55),
    ("s04", "Teaching + Benediction", ["11","12","13","14","15","16"], 200, 384, 57, 115),
]

# Assembled section MP3 filenames (in CHUNKS_DIR, already loudnorm-normalized)
SECTION_MP3 = {
    "s01": "NR_09_S01_Invocation_Humility.mp3",
    "s02": "NR_09_S02_Obedience.mp3",
    "s03": "NR_09_S03_Compassion.mp3",
    "s04": "NR_09_S04_Teaching_Benediction.mp3",
}

# Per-chunk sentence ranges (from monolithic CHUNKS manifest)
CHUNK_SENT_RANGES = {
    "01": (0,   27),
    "02": (28,  61),
    "03": (62,  85),
...
# Heading sentences: these are paraspan (h2/h3) sentences that are never
# narrated and therefore absent from CHUNK_TEXTS. They must be skipped
# during alignment so find_in_narration's failure cannot corrupt the pointer.
# Keys are sentence indices; values are the paragraph they open.
HEADING_SENTENCES = {
     0: 0,    # "Invocation" heading  — p0
    62: 17,   # "Mastery of Humility" — p17
    86: 24,   # "Mastery of Obedience" — p24  (NOTE: s86=p24 heading, s87=p25 first prose)
   151: 41,   # "Mastery of Compassion" — p41
   200: 56,   # "Mastery of Teaching" — p56
   236: 61,   # "Mastery of Meekness and Courage" — p61
   278: 74,   # "Mastery of Justice and Mercy" — p74
   301: 83,   # "Mastery of Joy" — p83
   331: 91,   # "Beatitudes as His Own Self-Portrait" — p91
   343: 96,   # "Benediction" — p96
}

# Full paragraph map: paragraph_index -> first_sentence_index
PARAGRAPH_MAP = {
...
# Narration texts sent to ElevenLabs (matches character alignment files exactly).
# Cue chunks 04, 07, 10, 17 are omitted — not used for prose alignment.
CHUNK_TEXTS = {
...
def find_in_narration(sentence_text, narration_text, search_from=0):
    """
    Find the character position of sentence_text's opening words within
    narration_text, starting from search_from.

    Returns (char_position, next_ptr, found):
      found=True  → match located; next_ptr advances past it
      found=False → no match; next_ptr == search_from (UNCHANGED)

    CRITICAL: on failure the pointer does NOT advance. This prevents heading
    sentences (never narrated, absent from chunk text) from falsely matching
    a word that appears later in the text, which would push search_from past
    the real positions of all following prose sentences.
    """
    sent_words = words_of(sentence_text)
    if not sent_words:
        return search_from, search_from, False
        ...
        if candidate == first_word:
            if second_word:
                ...
                if candidate2 == second_word:
                    return word_start, word_end, True
            else:
                return word_start, word_end, True

    # No match — return search_from UNCHANGED so next sentence is unaffected
    return search_from, search_from, False


def char_pos_to_time(char_pos, narration_text, chunk_chars, chunk_times):
    """
    Map a character position in narration_text to a timestamp via character walk.
    """
    if not chunk_times:
        return 0.0
    if char_pos <= 0:
...
print(f"  Heading sentences to skip: {sorted(HEADING_SENTENCES.keys())}")


# ── Step 2: Load alignment data for all prose chunks ──────────────────────

print("\n=== Step 2: Loading chunk alignment data ===")
chunk_alignment_cache = {}
all_prose_ids = set()
for _, _, chunk_ids, *_ in SECTIONS:
    all_prose_ids.update(chunk_ids)

for cid in sorted(all_prose_ids):
    path = os.path.join(ALIGNMENT_DIR, f"chunk_{cid}.json")
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    chunk_alignment_cache[cid] = {
        "chars":  data.get("characters", []),
        "starts": data.get("character_start_times_seconds", []),
    }
    print(f"  chunk_{cid}.json loaded: {len(chunk_alignment_cache[cid]['chars'])} chars")


# ── Step 3: Process each section ──────────────────────────────────────────

for sec_id, sec_label, chunk_ids, sent_start, sent_end, para_start, para_end in SECTIONS:

    print(f"\n{'='*60}")
    print(f"=== SECTION {sec_id.upper()}: {sec_label} ===")
    print(f"    chunks={chunk_ids}  sentences={sent_start}-{sent_end}  paras={para_start}-{para_end}")

    # 3a: Measure durations
    section_mp3_path = os.path.join(CHUNKS_DIR, SECTION_MP3[sec_id])
    assembled_duration = get_duration(section_mp3_path)
    print(f"\n  Assembled section MP3: {assembled_duration:.3f}s")

    chunk_pre_durations = []
    for cid in chunk_ids:
        dur = get_duration(os.path.join(CHUNKS_DIR, f"ch09_chunk_{cid}.mp3"))
        chunk_pre_durations.append(dur)
        print(f"  chunk_{cid} pre-norm: {dur:.3f}s")

    total_pre = sum(chunk_pre_durations)
    print(f"  Sum pre-norm: {total_pre:.3f}s  drift: {assembled_duration - total_pre:+.3f}s")
    ...
    for i, cid in enumerate(chunk_ids):
        ...
        chunk_actual_start = (cumulative_pre / total_pre) * assembled_duration
        chunk_actual_dur   = (pre_dur        / total_pre) * assembled_duration

        narration_text = CHUNK_TEXTS.get(cid, "")
        alignment      = chunk_alignment_cache[cid]
        chunk_chars    = alignment["chars"]
        chunk_starts   = alignment["starts"]

        first_s, last_s = CHUNK_SENT_RANGES[cid]
        narr_ptr = 0

        for sent_idx in range(first_s, last_s + 1):
            sent_text = sentences.get(sent_idx, "")

            # Heading sentences are never narrated — skip alignment entirely.
            # Assign them a placeholder time at the current narr_ptr position
            # (they won't appear as paragraph timestamps since PARAGRAPH_MAP
            # uses them only as the first sentence of a heading paragraph, and
            # heading paragraphs are paraspan elements skipped by highlightParagraph).
            if sent_idx in HEADING_SENTENCES:
                # Use current narr_ptr's approximate time — heading p{N} gets
                # overridden to 0.0 (para_start) or simply not seeked to in JS.
                chunk_time  = char_pos_to_time(narr_ptr, narration_text, chunk_chars, chunk_starts)
                global_time = chunk_actual_start + (chunk_time / pre_dur) * chunk_actual_dur
                sentence_timestamps[sent_idx] = round(global_time, 3)
                skipped_headings.append(sent_idx)
                continue   # ← pointer NOT advanced

            char_pos, new_ptr, found = find_in_narration(sent_text, narration_text, narr_ptr)

            if found:
                narr_ptr = new_ptr
            else:
                failed_matches.append(sent_idx)
    ...
    if skipped_headings:
        print(f"  Headings skipped (correct): {skipped_headings}")
    if failed_matches:
        print(f"  WARNING: {len(failed_matches)} sentences failed match: {failed_matches[:10]}")

    # 3c: Build paragraph timestamps (section scope only)
    section_para_map = {
        p: fs for p, fs in PARAGRAPH_MAP.items()
        if para_start <= p <= para_end
    }
    para_timestamps = {}
    for para_idx, first_s in sorted(section_para_map.items()):
        if first_s in sentence_timestamps:
            para_timestamps[para_idx] = sentence_timestamps[first_s]
        else:
            print(f"  WARNING: s{first_s} (para {para_idx}) missing from sentence_timestamps")

    # Force first prose paragraph to 0.0 (silence pre-roll)
    para_timestamps[para_start] = 0.0
    print(f"  p{para_start} forced to 0.0 (silence pre-roll)")
    print(f"  {len(para_timestamps)} paragraph timestamps")

    # 3d: Build sentence-to-paragraph lookup (section scope)
    para_indices = sorted(section_para_map.keys())
    sentence_to_para = {}
    for i, para_idx in enumerate(para_indices):
        first_s_in_para = section_para_map[para_idx]
        next_first = section_para_map[para_indices[i + 1]] if i + 1 < len(para_indices) else (sent_end + 1)
        for s in range(first_s_in_para, next_first):
            if sent_start <= s <= sent_end:
                sentence_to_para[s] = para_idx

    print(f"  {len(sentence_to_para)} sentence-to-paragraph mappings")

    # 3e: Assemble and write JSON
    output = {}
    ...
    out_path = os.path.join(TIMESTAMPS_DIR, f"chapter-09-{sec_id}.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2)

    print(f"  Written: {out_path}")

    # 3f: Monotonicity sanity check — ALL paragraphs, not just the first 8
    print(f"\n  Paragraph timestamps (monotonicity check):")
    para_keys = sorted([k for k in output if k.startswith("p")], key=lambda x: int(x[1:]))
    prev_t  = -1.0
    mono_ok = True
    for pk in para_keys:
        pt       = output[pk]
        ...
        para_num = int(pk[1:])
        first_s_idx = section_para_map.get(para_num)
        preview  = sentences.get(first_s_idx, "")[:35] if first_s_idx is not None else ""
        mins     = int(pt // 60)
        secs     = pt % 60
        print(f"    {pk:>5} = {pt:>8.3f}s ({mins}:{secs:05.2f})  {status}  {preview}")
        if not ok:
            mono_ok = False
        prev_t = pt
    print(f"  Monotonicity: {'PASS' if mono_ok else 'FAIL — review alignment'}")
...
for sec_id, sec_label, *_ in SECTIONS:
    p    = os.path.join(TIMESTAMPS_DIR, f"chapter-09-{sec_id}.json")
    size = os.path.getsize(p) if os.path.exists(p) else 0
    print(f"  chapter-09-{sec_id}.json  ({size:,} bytes)  [{sec_label}]")

print("""
Next steps:
  git add src/_data/timestamps/chapter-09-s*.json
  git commit -m "Ch. 9: fix timestamp alignment — heading false-match bug"
  git push
  Hard reload Ch. 9 — test click-to-seek in Humility section
""")
