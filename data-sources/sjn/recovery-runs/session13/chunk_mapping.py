"""Session 13, phase 4 (session 12 phase 3 script, re-run): the chunk-id mapping for the five re-chunked rows, and every stored candidate located on a changed chunk.
No model calls, no network.

  python data-sources/sjn/recovery-runs/session13/chunk_mapping.py <dir holding the pre-re-chunk <rid>.json files>

Writes session13/chunk-id-mapping.json:
