/**
 * articles_apparatus.js — Eleventy JS data file
 * Reads all 13 per-article YAML files and returns the apparatus data object.
 * Available in templates as {{ apparatusData }}.
 *
 * Build note: The YAML source lives outside the repo in the working-folder.
 * During local development this file resolves directly.
 * For Vercel deployment, ensure YAML_DIR is accessible or commit a static
 * articles_apparatus.json instead (which takes precedence over this .js file).
 */
...
// ── YAML source directory ────────────────────────────────────────────────────
// Adjust if the repo moves machines. Vercel builds will skip if not found
// and fall back to articles_apparatus.json if that file is committed.
...
module.exports = function buildApparatus() {
  // If YAML dir isn't accessible (Vercel build), return empty object.
  // The static articles_apparatus.json will be used instead if committed.
  if (!fs.existsSync(YAML_DIR)) {
    console.warn('[articles_apparatus.js] YAML dir not found — returning {}');
    return {};
  }
  ...
  for (const [aid, fname] of Object.entries(ARTICLE_FILES)) {
    ...
    if (!fs.existsSync(filePath)) {
      console.warn(`[articles_apparatus.js] Missing: ${fname}`);
      apparatus[aid] = {};
      continue;
    }

    let raw = fs.readFileSync(filePath, 'utf-8');
    // Strip trailing --- document separator if present
    raw = raw.replace(/\n---\s*$/, '').trim();
