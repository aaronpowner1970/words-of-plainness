...
{% sentence 2 %}The previous chapter explored the four natural faculties by which all human beings acquire knowledge: senses, authority, reason, and intuition.{% endsentence %} {% sentence 3 %}These faculties are gifts from God, and through them we have built civilizations, cured diseases, and explored the cosmos.{% endsentence %} {% sentence 4 %}Yet we also discovered their ceiling—there are questions that exceed their reach.{% endsentence %}

{% sentence 5 %}As Latter-day Saints, we affirm that a fifth faculty exists, one that completes the other four.{% endsentence %} {% sentence 6 %}This faculty does not contradict natural knowledge; it bridges the gap where senses, authority, reason, and academic intuition reach their limit.{% endsentence %} {% sentence 7 %}We call this faculty *revelation*, and it operates through the Spirit of God communing with the spirit within each person.{% endsentence %}
...
{% sentence 27 %}The physical human brain is definitely part of the current state of the human mind, but it is not the source of human intelligence; though it seems to be required to join and coordinate intelligence with the physical body.{% endsentence %} {% sentence 28 %}Philosophers and scientists agree that the human mind is one of the most complex, layered systems in existence.{% endsentence %} {% sentence 29 %}Yet there is much debate over what the mind actually is and what it actually does.{% endsentence %}
...
- {% sentence 164 %}Choose humility over stubborn pride.{% endsentence %}
- {% sentence 165 %}Learn to let go of fears and embrace personal growth.{% endsentence %}
- {% sentence 166 %}Thirst for and actively seek truth.{% endsentence %}
- {% sentence 167 %}Build a desire to live right, taking steps to become more righteous.{% endsentence %}
- {% sentence 168 %}Regularly devote time to quiet meditation and prayer.{% endsentence %} {% sentence 169 %}Connecting with and recognizing the sensations of God's Spirit require an effort to reach and maintain a certain state of mind and heart.{% endsentence %} {% sentence 170 %}Relaxation, quieting your mind, reaching out to the higher power of God, and maintaining openness are essential.{% endsentence %}
- {% sentence 171 %}Give honest consideration to the teachings of scriptures, and to the words of prophets and apostles of God.{% endsentence %}
...
{% sentence 253 %}We must pause to evaluate ourselves periodically.{% endsentence %} {% sentence 254 %}Embracing extremes of pride, despair, rebellion, and submission can cripple the human soul, and we often seek to excuse these attitudes and behaviors through self-justification.{% endsentence %} {% sentence 255 %}Extremes in general are to be avoided in dealing with life.{% endsentence %} {% sentence 256 %}We feel that in many cases health and wisdom lay somewhere in between extremes.{% endsentence %} {% sentence 257 %}Between pride and despair is humility.{% endsentence %} {% sentence 258 %}At the balancing point between rebellion and submission is the exercising of careful judgment.{% endsentence %}
